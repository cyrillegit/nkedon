...
(function($){$.fn.inlinemultiselect=function(options){var options=$.extend({separator:", ",separatorSpecial:{},endSeparator:"or",endSeparatorSpecial:{},showAllLink:true,showNoneLink:true,showResetLink:true,triggerPopup:{'empty':"[Choose...]",'nonempty':"[Change...]",'disabled':"[Editing disabled]"},wrapperClass:'selectWrapper',wrapperClassDisabled:'selectWrapperDisabled',controlsClass:'selectControl',listWrapperClass:'listWrapper',checkboxClass:'listBox',checkedClass:'boxChecked',uncheckedClass:'boxUnchecked',disabledClass:'boxDisabled',hoverClass:'boxHover',maxSelected:{},changeCallback:null,showConsoleLog:true},options||{});var logToConsole=function(msg){if(typeof console=="undefined"||!options.showConsoleLog){return;}else{console.log(msg);}};return this.each(function(){if(this.type==='select-multiple'){var msname=$(this).attr('name').replace(/\[\]$/,'');logToConsole("Found multiple select with name: "+msname);var s=options.separatorSpecial[msname]?options.separatorSpecial[msname]:options.separator;var es=options.endSeparatorSpecial[msname]?options.endSeparatorSpecial[msname]:options.endSeparator;logToConsole("separator for "+msname+": "+s);logToConsole("end separator for "+msname+": "+es);var wc=(this.disabled)?options.wrapperClassDisabled:options.wrapperClass;var wrapper=$('<div id="'+msname+'"></div>').addClass(wc);var controls=$('<div/>').appendTo(wrapper)
var optionContainer=$('<div/>').addClass(options.listWrapperClass).appendTo(wrapper);var selectList=$('<ul/>').appendTo(optionContainer);$('<a href="#">Fermer</a>').addClass(options.controlsClass).click(function(){$(this).parents('.selectWrapper').hide();this.blur();return false;}).appendTo(controls);if(options.showResetLink){$('<a href="#">Ràz</a>').addClass(options.controlsClass).click(function(){var checkboxgroup=$(this).parents('.'+options.wrapperClass).find(':checkbox');checkboxgroup.each(function(){this.checked=this.defaultChecked;});checkboxgroup.each(function(){$(this).trigger('change');});this.blur();return false;}).appendTo(controls);}
if(options.showAllLink){$('<a href="#">Tous</a>').addClass(options.controlsClass).click(function(){var checkboxgroup=$(this).parents('.'+options.wrapperClass).find(':checkbox');var groupID=$($(this).parents('.'+options.wrapperClass).get(0)).attr('id');if(options.maxSelected[groupID]&&options.maxSelected[groupID]<checkboxgroup.length){alert("Sorry, there is a limit of "+options.maxSelected[groupID]+" possible selection(s) from this list");}else{checkboxgroup.each(function(){if(!this.disabled){this.checked=true;}});checkboxgroup.each(function(){$(this).trigger('change');});}
this.blur();return false;}).appendTo(controls);}
if(options.showNoneLink){$('<a href="#">Aucun</a>').addClass(options.controlsClass).click(function(){$(this).parents('.'+options.wrapperClass).find(':checkbox').each(function(){this.checked=false;$(this).trigger('change');});this.blur();return false;}).appendTo(controls);}
