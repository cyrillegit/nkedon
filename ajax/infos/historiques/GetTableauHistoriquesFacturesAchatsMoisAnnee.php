...
if(  $date_histo_facture == "" ) {
    $histo_factures = $db->getAllFacturesAchatsByJourMoisAnnee( "", $mois, $annee );
}else{
    $jour = explode( "/", $date_histo_facture )[0];

    $histo_factures = $db->getAllFacturesAchatsByJourMoisAnnee( $jour, $mois, $annee );
}
...
if( COUNT($histo_factures) > 0 )
{
    ...
    {
        $produitsFacture = $db->getAllHistoriquesAchatsByFacture( $facture["idt_factures_achats"] );
